...
Step3 - clean/wrangle fracking data
```{r}
fracking_clean <- fracking |> 
  
  # clean column names ----
  janitor::clean_names() |> # update column header to snake case "_"
  ...
  mutate(year = year(datetime_start)) |> # create 'year' col from date with xxxx

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> # select the columns only for using
  
  # filter out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) |> # remove the not state names
  
  # rename state_name to something shorter for typing out when using case_when (not necessary) ----
  rename(sn = state_name) |> # temporary making the columns names 'sn' to distinguish for later
  
  # make all words title case ----
  mutate(sn = str_to_title(sn)) |> # making first letter is capital
  
  # fix misspelled state names ---- # identify the wrong one and right one
  mutate(sn = case_when(
  ...
  mutate(sn = str_to_title(sn)) |> # first letter is capital
  
  # create a column of just state abbreviations ----
  mutate(state_abb = usdata::state2abbr(sn)) |> # convert 'sn' column as state abbreviatation
  
  # rename 'sn' to 'state_name' again for clarity ----
  rename(state_name = sn, total_base_water_volume_gal = total_base_water_volume) |> # rename the column
